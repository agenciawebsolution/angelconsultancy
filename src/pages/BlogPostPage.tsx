  ...
  if (!post) {
    return (
      <div className="pt-36 pb-32 max-w-xl mx-auto px-4 text-center space-y-4">
        ...
        <h1 className="text-2xl font-bold text-slate-900">{blogT.notFoundTitle}</h1>
        <p className="text-sm text-slate-500">
          {blogT.notFoundSubtitle}
        </p>
        <Link
          to="/blog"
    ...
    <article className="pt-28 sm:pt-32 pb-20 lg:pb-28 bg-[#FAFBFD] min-h-screen">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
          <Link to="/" className="hover:text-brand-navy transition-colors">{translations.nav.home}</Link>
          <span>/</span>
          <Link to="/blog" className="hover:text-brand-navy transition-colors">{translations.nav.blog}</Link>
          <span>/</span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-200/70 text-xs text-slate-500">
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {post.published_at ? new Date(post.published_at).toLocaleDateString(language, { day: '2-digit', month: 'long', year: 'numeric' }) : ''}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] font-semibold text-slate-400 mr-1 flex items-center gap-1">
                <Share2 className="w-3.5 h-3.5" /> {blogT.shareArticle}:
              </span>
              <a
        ...
        <div className="p-8 rounded-3xl bg-brand-navy text-white text-center space-y-4 shadow-soft-lg">
          <h3 className="text-xl sm:text-2xl font-bold">{translations.contact.formTitle}</h3>
        ...
        {related.length > 0 && (
          <div className="pt-8 space-y-6">
            <h3 className="text-xl font-bold text-slate-900">{blogT.relatedArticles}</h3>
